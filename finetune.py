        ...
        try:
            ...
            for j, orig_token in enumerate(labels[source_len:]):
                if orig_token == context_markups[0]:
                    context_start = source_len + j
                    break

            if context_start is not None:
                context_end = None
                for k, orig_token in enumerate(labels[context_start + 1:]):
                    if orig_token == context_markups[1]:
                        context_end = context_start + 1 + k
                        break
            ...
            print(f"[Warning] Failed to apply context markup masking: {e}")
            pass
...
def main():
    ...
    if args.config_name:
        config = TransformerConfig.from_pretrained(args.config_name)
    elif args.model_name_or_path:
        config = TransformerConfig.from_pretrained(args.model_name_or_path)
    else:
        raise ValueError(
    ...
    if args.use_special_tokens:
        context_tokens = ["<title>", "<body>", "<table>", "</table>", "<item>", "</item>", "<description>", "</description>"]
        tokenizer.add_tokens(context_tokens, special_tokens=True)
        context_markups = [tokenizer.convert_tokens_to_ids(tok) for tok in context_tokens]
    else:
    ...
    if args.model_name_or_path:
        ...
        model = TransformerForCausalLM.from_config(config)
    ...
    with accelerator.main_process_first():
        ...
        lm_datasets = lm_datasets.filter(
            lambda example: (example['labels'] != -100).any() and (example['input_ids'] != tokenizer.pad_token_id).any()
        )
