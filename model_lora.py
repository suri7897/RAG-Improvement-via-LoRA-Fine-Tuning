...
## This is almost same as model.py, and slightly different for application of lora_finetuning. 
    ...
    def forward(
        ...
        **kwargs
        ...
        if labels is not None:
            ...
            real_vocab_size = shift_logits.size(-1)

            if shift_logits.numel() % real_vocab_size != 0:
                raise ValueError(
                    f"Cannot reshape shift_logits of size {shift_logits.numel()} into [-1, {real_vocab_size}]"
                )

            loss = F.cross_entropy(
                shift_logits.contiguous().view(-1, real_vocab_size),
